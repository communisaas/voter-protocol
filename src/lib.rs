#![no_std] // exclude standard library from build
use soroban_sdk::{contractimpl, contracttype, Address, BytesN, Env, IntoVal, RawVal};

mod token {
    soroban_sdk::contractimport!(file = "./tokenSpec.wasm");
}
...
/*
TODO: Contract to deliver a read receipt

1. Call initialize(recipient, mailHash, receiptBody, token).
2. Email read events send tokens to this contract's address
3. Token is available for recipient to withdraw.
*/
#[contractimpl]
...
impl Receipt {
    pub fn initialize(
        ...
        e.storage().set(&DataKey::MailHash, &mailHash);
        e.storage().set(&DataKey::ReceiptBody, &receiptBody);
