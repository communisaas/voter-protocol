#!/usr/bin/env python3
"""
IMPROVED NLP Semantic Validation for Council District Training Data
Version 2: Fixed scoring bugs and enhanced pattern detection
"""
...
class CouncilDistrictSemanticAnalyzerV2:
    """
    Enhanced semantic analysis with improved scoring

    Key improvements over V1:
    1. Stronger penalties for "by Council District" pattern
    2. Better state-level council detection
    3. Named entity detection for person names
    4. Form/survey detection enhancements
    """
    ...
    def __init__(self):
        ...
        self.positive_patterns = {
            'explicit_boundaries': [
                # These are nearly definitive positive signals
                r'\bward\s+boundaries',
                r'\bcouncil.*district.*boundaries',
                r'\bdistrict.*boundaries',
                r'\baldermanic.*boundaries',
            ],
            'primary_council': [
        ...
        self.negative_patterns = {
            # CRITICAL DISQUALIFIERS (nearly definitive)
            'data_aggregation': [
                # These patterns are nearly definitive - data ABOUT districts, not boundaries
                r'\bby\s+(.*?\s+)?council\s+district',  # "by Atlanta City Council District"
                r'\bper\s+(.*?\s+)?council\s+district',
                r'\baggregate.*by.*district',
                r'\btotal.*by.*district',
            ],

            'state_level_council': [
                # State-level councils (not city council districts)
                r'\bgovernor.*council',
                r'\bexecutive\s+council\s+district',  # State executive councils
                r'\bstate.*executive.*council',
            ],
            ...
            'other_districts': [
                ...
                r'\bwater\s+district',
                r'\bpark\s+district',
                r'\bsupervisory\s+district',  # County supervisory districts
            ],

            'data_topics': [
                # Dataset is ABOUT a topic, not boundaries
                r'\bcrime\b',
                r'\bhousing\b',
                r'\bvehicle\b',
                r'\btransport',
            ...
            'data_descriptors': [
                # Contextual words indicating data ABOUT districts
                r'\bin\s+(.*?\s+)?council',  # "in Penrith City Council"
                r'\bfor\s+(.*?\s+)?council\s+district',  # "for District 11"
                r'\bat\s+(.*?\s+)?council',  # "at Hamilton City Council"
                r'\bfrom\s+(.*?\s+)?council',
                r'\bwithin\s+council\s+district',
                r'\banalysis.*council',
                r'\bstudy.*council',
                r'\bsurvey.*council',
                r'\bdata.*council',
        ...
        self.context_patterns = {
            'single_district_number': r'\bcouncil\s+district\s+\d+\b',
            'aggregated_data': r'\b(total|sum|count|average|median).*\bdistrict',
            'year_suffix': r'\bdistrict.*20\d{2}\b',
    ...
    def analyze(self, title: str, fields: List[str], description: str) -> SemanticAnalysis:
        ...
        # Check positive patterns with NEW WEIGHTS
        for category, patterns in self.positive_patterns.items():
            for pattern in patterns:
                if re.search(pattern, title_lower, re.IGNORECASE):
                    positive_signals.append(f"Positive:{category}:{pattern}")

                    # NEW WEIGHTS - more conservative
                    if category == 'explicit_boundaries':
                        confidence_score += 30  # Very strong signal
                    elif category == 'primary_council':
                        confidence_score += 15  # Reduced from 20
                    elif category in ['ward_patterns', 'aldermanic_patterns']:
                        confidence_score += 12
                    elif category == 'official_indicators':
                        confidence_score += 8   # Reduced from 10

        # Check negative patterns with ENHANCED WEIGHTS
        for category, patterns in self.negative_patterns.items():
            for pattern in patterns:
                if re.search(pattern, title_lower, re.IGNORECASE):
                    negative_signals.append(f"Negative:{category}:{pattern}")

                    # NEW WEIGHTS - much stronger penalties for critical disqualifiers
                    if category == 'data_aggregation':
                        confidence_score -= 50  # CRITICAL: "by District" nearly definitive
                    elif category == 'state_level_council':
                        confidence_score -= 45  # State-level councils are not city councils
                    elif category == 'other_districts':
                        confidence_score -= 35
                    elif category == 'data_topics':
                        confidence_score -= 30  # Strong signal it's data, not boundaries
                    elif category == 'data_descriptors':
                        confidence_score -= 25
                    elif category == 'form_surveys':
                        confidence_score -= 40  # Forms are not boundaries
                    elif category in ['non_governance', 'analysis_datasets']:
                        confidence_score -= 20
        ...
        if has_council_district and has_data_topic:
            ...
            confidence_score -= 45  # Very strong signal this is NOT boundaries

        # Single district number (ambiguous)
        if re.search(self.context_patterns['single_district_number'], title_lower):
            ...
            if re.search(self.context_patterns['boundaries_explicit'], combined):
                ...
                confidence_score += 10  # Increased from 5
            else:
                context_signals.append("Context:single_district_data_uncertain")
                confidence_score -= 10  # Increased penalty from 5
    ...
    def _analyze_fields(self, fields: List[str]) -> List[Tuple[str, int]]:
        ...
        # Positive field indicators (boundary-specific fields)
        if 'district_n' in fields_text or 'district_number' in fields_text or 'district_id' in fields_text:
            signals.append(('district_identifier_field', 15))
        if 'ward' in fields_text and not any(x in fields_text for x in ['award', 'steward', 'toward']):
            signals.append(('ward_field', 12))
        if 'alderman' in fields_text or 'councilmember' in fields_text or 'councilor' in fields_text:
            signals.append(('representative_field', 18))

        # Negative field indicators (data fields, not boundaries)
        demographic_fields = ['population', 'demographic', 'census', 'income', 'poverty', 'median', 'average',
                              'male', 'female', 'age', 'race', 'ethnicity', 'married', 'divorced', 'education']
        if any(f in fields_text for f in demographic_fields):
            signals.append(('demographic_field', -20))

        parcel_fields = ['ain', 'apn', 'parcel', 'situs', 'taxrate', 'yearbuilt', 'usecode']
        if any(f in fields_text for f in parcel_fields):
            signals.append(('parcel_field', -25))

        infrastructure_fields = ['fire', 'school', 'utility', 'water', 'sewer', 'electric']
        if any(f in fields_text for f in infrastructure_fields):
            signals.append(('infrastructure_field', -18))
...
def compare_classifications(
    samples: List[Dict],
    analyzer: CouncilDistrictSemanticAnalyzerV2
) -> Dict:
    """
    ...
    stats = {
        ...
        'false_positives_fixed': [],  # Track V1 false positives that V2 fixes
    }
...
def print_report(stats: Dict):
    ...
    print("=" * 80)
    print("SEMANTIC VALIDATION REPORT (Version 2 - Improved Scoring)")
    print("=" * 80)
    ...
    for i, case in enumerate(stats['semantic_confident_corrections'][:15], 1):
        print(f"\n{i}. {case['title']}")
        print(f"   LLM: {case['llm_label']} → Semantic: {case['semantic_label']} (confidence: {case['confidence']}%)")
        print(f"   Reasoning: {case['reasoning'][:150]}")

    if len(stats['semantic_confident_corrections']) > 15:
        print(f"\n   ... and {len(stats['semantic_confident_corrections']) - 15} more")
...
def main():
    ...
    print("\nInitializing semantic analyzer V2 (improved scoring)...")
    analyzer = CouncilDistrictSemanticAnalyzerV2()
    ...
    output_path = '/Users/noot/Documents/voter-protocol/packages/crypto/services/shadow-atlas/agents/langgraph/semantic_validation_v2_results.json'
