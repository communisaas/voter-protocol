...
import { getTopologyRules, type TIGERLayerType } from './topology-rules.js';
  ...
  for (const boundary of boundaries) {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        console.warn(
          `Intersection failed for ${boundary.id} and ${candidate.geoid}`
        );
      }
    }
  }
...
export function detectGaps(
  ...
  for (let i = 1; i < validChildren.length; i++) {
    ...
    try {
      ...
      console.warn(`Union failed for child ${validChildren[i].id}`);
    }
  }
