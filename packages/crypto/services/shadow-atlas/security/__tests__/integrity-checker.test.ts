...
describe('Integrity Checker - Merkle Proof Verification', () => {
  test('verifies valid Merkle proof', () => {
    // Simple 2-level tree
    const leaf = 123n;
    const sibling = 456n;
    ...
    // Real tests would use known-good test vectors from Barretenberg
    const result = verifyMerkleProof(leaf, [], [], leaf); // Single-node tree
  ...
  test('rejects invalid Merkle proof', () => {
    ...
    const result = verifyMerkleProof(leaf, siblings, pathIndices, wrongRoot);
    expect(result).toBe(false);
  });

  test('rejects mismatched siblings and path indices', () => {
    const leaf = 123n;
    const siblings = [456n, 789n];
    const pathIndices = [0]; // Too few indices

    const result = verifyMerkleProof(leaf, siblings, pathIndices, 999n);
    expect(result).toBe(false);
  });

  test('handles single-node tree (leaf is root)', () => {
    const leaf = 123n;
