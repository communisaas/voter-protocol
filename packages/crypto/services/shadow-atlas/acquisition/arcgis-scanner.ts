...
/**
 * State bounding boxes for geographic filtering (approximate)
...
/**
 * Known city portal patterns (manually curated official sources)
 */
interface KnownPortalEntry {
  readonly socrata?: string; // Socrata domain
  readonly arcgis?: string;   // ArcGIS Hub domain
  readonly datasetId?: string; // Known dataset ID
}

const KNOWN_CITY_PORTALS: Record<string, KnownPortalEntry> = {
  '5363000': {  // Seattle, WA
...
export class ArcGISScanner {
   ...
   */
  private async enumerateLayers(serviceUrl: string): Promise<readonly Array<{
    readonly id: number;
    readonly name: string;
    readonly type: string;
