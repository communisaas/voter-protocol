...
export class CountyGeometryService {
   ...
   *
   * Uses Census PLACE-to-county crosswalk
   *
   * TODO: Implement actual Census crosswalk lookup
   * https://www2.census.gov/geo/docs/maps-data/data/rel2023/place_county/place_county.txt
   *
   * @param cityFips - 7-digit city FIPS code
   */
  private async inferPrimaryCounty(cityFips: string): Promise<string> {
    // For single-county cities not in registry, we need the crosswalk
    // For now, throw error to indicate missing data

    // Extract state FIPS (first 2 digits)
    const stateFips = cityFips.slice(0, 2);

    // Mock implementation for testing
    // Example: Boulder, CO (0803000) -> Boulder County (08013)
    if (cityFips === '0803000') {
      return '08013'; // Boulder County, CO
    }

    throw new Error(
      `Cannot infer primary county for city FIPS ${cityFips}. ` +
      `Add to multi-county registry or implement Census crosswalk lookup.`
    );
  }
