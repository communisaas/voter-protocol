/**
 * Shadow Atlas Merkle Tree v2 - Parallel Implementation
 *
 ...
 *
 * MIGRATION NOTE:
 * This replaces the synchronous merkle-tree.ts implementation.
 * The async API is required because Noir.execute() is async.
 */
 ...
 * USAGE:
 * ```typescript
 * const tree = await ShadowAtlasMerkleTreeV2.create(addresses);
 * const proof = tree.generateProof(address);
 * const isValid = await tree.verifyProof(proof, address);
 * ```
 */
export class ShadowAtlasMerkleTreeV2 {
   ...
   * @returns Promise<ShadowAtlasMerkleTreeV2>
   */
  static async create(
    addresses: readonly string[],
    config: MerkleTreeConfig = {}
  ): Promise<ShadowAtlasMerkleTreeV2> {
    ...
    const layers = await ShadowAtlasMerkleTreeV2.buildTreeParallel(
      leaves,
      depth,
      hasher,
      batchSize
    ...
    return new ShadowAtlasMerkleTreeV2(leaves, layers, hasher, depth, addressToIndex);
  }
...
export async function createShadowAtlasMerkleTree(
  ...
  return ShadowAtlasMerkleTreeV2.create(addresses, config);
}
