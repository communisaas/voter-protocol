...
//! Single-tier K=14 circuit removes global tree, moves district→country mapping
...
// 🔒 SECURITY: Single-tier circuit with on-chain registry
...
fn load_ceremony_params_for_verifier(k: u32) -> Result<ParamsKZG<Bn256>, String> {
    ...
    // 🔒 CRITICAL: Use Axiom challenge_0085 params for snark-verifier compatibility
    ...
    // 🔒 SECURITY: Verify integrity with Blake2b hash (defense against supply-chain attacks)
    ...
    match k {
        12 => {
            // 🚨 TODO: Update with actual K=12 canonical hash from Ethereum ceremony
            // For now, log hash for manual verification
            eprintln!("⚠️  K=12 hash verification: Canonical hash not yet recorded");
            ...
            //     return Err(format!("🚨 K=12 parameter hash mismatch: {}", hash_hex));
            // }
        }
        14 => {
            if CANONICAL_HASH_K14 == "PLACEHOLDER_WILL_BE_COMPUTED" {
                ...
                eprintln!("   Computed hash: {}", hash_hex);
                eprintln!("   📋 ACTION REQUIRED: Update CANONICAL_HASH_K14 in generate_verifier.rs");
            } else if hash_hex != CANONICAL_HASH_K14 {
                return Err(format!("🚨 K=14 parameter hash mismatch: Expected {}, got {}", CANONICAL_HASH_K14, hash_hex));
            } else {
                eprintln!("✅ Hash verification PASSED - Parameters are Axiom challenge_0085 canonical");
            }
        }
        _ => {
            eprintln!("⚠️  No canonical hash recorded for K={}, skipping verification", k);
            eprintln!("   Computed hash: {}", hash_hex);
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔧 Generating EVM verifier for district membership circuit...");
    println!();

    // 1. Load KZG ceremony parameters (same as prover)
    println!("📦 Loading KZG ceremony parameters (k={})...", K);

    let params = load_ceremony_params_for_verifier(K)?;
    println!("✅ Loaded ceremony parameters ({} elements)", params.n());
    println!();

    // 2. Create circuit for keygen (determines shape)
    println!("🏗️  Building circuit for keygen...");
    ...
    // 🔒 SECURITY: Create dummy circuit with SINGLE-TIER structure
    ...
    println!("📊 Circuit parameters calculated:");
    ...
    println!("   - 🚨 CRITICAL: Bytecode size ~ O(num_columns), not O(num_rows)");
    println!();
    ...
    println!("🔑 Generating proving key with wrapped circuit (this takes ~30-60 seconds)...");
    ...
    // 🔒 Extract break_points from the circuit_for_keygen's builder
    // After keygen, the builder inside the wrapped circuit has break_points set
    println!("📊 Extracting break points from wrapped circuit's builder...");
    let break_points = circuit_for_keygen.builder.break_points();
    println!("✅ Break points extracted ({} phases)", break_points.len());
    println!();

    // Save PK to disk for prover to load
    println!("💾 Saving proving key to disk...");
    ...
    println!("✅ Break points saved to: {}", break_points_path.display());
    println!();

    // 5. Generate EVM verifier using VK from the proving key
    // 🔒 CRITICAL: We MUST use pk.get_vk() here, not a separately generated VK
    ...
    println!("⚙️  Generating EVM verifier bytecode from proving key...");
    ...
    // ✅ Using same circuit_for_keygen that was used for PK generation
    ...
    println!("✅ Verifier bytecode generated ({} bytes)", deployment_code.len());
    println!();
    ...
    println!("💾 Written unoptimized bytecode to: {}", bytecode_path.display());
...
"#,
    ...
    println!("📝 Written deployment info to: {}", info_path.display());
    println!();

    // 8. Success summary
    println!("═══════════════════════════════════════════════════════════");
    println!("✅ VERIFIER GENERATION COMPLETE");
    println!("═══════════════════════════════════════════════════════════");
    println!();
    ...
    if deployment_code.len() > eip170_limit {
        ...
        println!("⚠️  BYTECODE SIZE WARNING:");
        ...
        println!();
        println!("  ✅ Fits EIP-170 limit - can deploy directly to Ethereum/Scroll");
        println!();
    }
    ...
    println!();
    println!("🔒 SECURITY: This verifier will verify ALL district proofs on-chain.");
