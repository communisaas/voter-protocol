/**
 * DeepSeek model provider (DeepSeek V3.2).
 *
...
export class DeepSeekProvider implements ModelProvider {
	...
	constructor(apiKey: string, modelName = 'deepseek-v3.2', baseUrl = 'https://api.deepseek.com/v1') {
		this.apiKey = apiKey;
	...
	private parseEvaluations(
		...
		if (!validation.valid) {
			throw new Error(
				`DeepSeek output validation failed: ${validation.errors.join('; ')}`,
			);
		}
