 ...
 *
 * Demonstrates GEOID completeness checking for TIGER data extraction.
 */
...
import {
  ...
  parseGEOID,
} from '../src/registry/expected-geoids.js';

// ============================================================================
// Example 1: Generate Expected GEOIDs
// ============================================================================

console.log('='.repeat(80));
console.log('Example 1: Generate Expected GEOIDs');
console.log('='.repeat(80));

// Get expected congressional districts for California
const caCongressional = getExpectedCongressionalGEOIDs('06');
console.log(`\nCalifornia Congressional Districts: ${caCongressional.length}`);
console.log(`First 5: ${caCongressional.slice(0, 5).join(', ')}`);
console.log(`Last 5:  ${caCongressional.slice(-5).join(', ')}`);

// Get expected counties for Texas
const txCounties = getExpectedCountyGEOIDs('48');
console.log(`\nTexas Counties: ${txCounties.length} (most in US)`);
console.log(`First 5: ${txCounties.slice(0, 5).join(', ')}`);
console.log(`Last 5:  ${txCounties.slice(-5).join(', ')}`);

// Get at-large district for Alaska
const akCongressional = getExpectedCongressionalGEOIDs('02');
console.log(`\nAlaska Congressional Districts: ${akCongressional.length} (at-large)`);
console.log(`District: ${akCongressional[0]}`);
