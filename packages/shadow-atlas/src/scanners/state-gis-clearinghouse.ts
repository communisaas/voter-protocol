...
export class StateGISClearinghouseScanner {
  ...
  private async exploreServicesRecursively(
    ...
    if (depth >= 5) {
      console.log(`   ⚠️  Max depth reached at: ${folder || '(root)'}`);
      return [];
    }
    ...
    if (skipFolders.some(skip => folderLower.includes(skip))) {
      console.log(`   ⏭️  Skipping non-governance folder: ${folder}`);
      return [];
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`   ⚠️  Failed to fetch folder: ${folder || '(root)'} (${response.status})`);
        return [];
      }
      ...
      console.warn(`   ⚠️  Error exploring folder ${folder || '(root)'}: ${(error as Error).message}`);
    }
  ...
  private async exploreService(
    ...
    try {
      ...
      console.warn(`   ⚠️  Error exploring service ${serviceUrl}: ${(error as Error).message}`);
      return [];
    }
  }
  ...
  private async evaluateLayer(
    ...
    try {
      ...
      if (score.score < 30) {
        console.log(`   ⏭️  Layer rejected: ${layerData.name} (score: ${score.score})`);
        console.log(`      Reasons: ${score.reasons.join(', ')}`);
        return null;
      }
      ...
      console.log(`   ✅ Governance layer found: ${layerData.name} (score: ${score.score + 15})`);
      ...
      console.warn(`   ⚠️  Error evaluating layer ${layerId}: ${(error as Error).message}`);
      return null;
    }
  }
