...
export class RDHScanner {
  ...
  async scan(options: RDHScannerOptions = {}): Promise<RDHScanResult> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[RDHScanner] Failed to fetch download page: ${response.status}`
        );
        return {
          datasetsFound: [],
          newReleases: [],
      ...
      console.error(`[RDHScanner] Scan failed: ${errorMessage}`);
  ...
  async listDatasetsAuthenticated(
    ...
    if (!this.credentials) {
      console.warn('[RDHScanner] No credentials - falling back to web scraping');
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[RDHScanner] API request failed: ${response.status} ${response.statusText}`
        );
        return [];
      }
      ...
      if (!Array.isArray(data)) {
        ...
        console.error(
          `[RDHScanner] API error for ${stateCode}:`,
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`[RDHScanner] API call failed: ${errorMessage}`);
      return [];
    }
  }
