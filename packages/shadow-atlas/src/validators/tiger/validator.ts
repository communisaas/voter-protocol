...
export class TIGERValidator {
  ...
  validateCompleteness(
    ...
    if (layer === 'cd') {
      ...
      if (stateFips && this.isAtLargeCongressionalState(stateFips)) {
        ...
        if (filtered.length !== 1 && filtered.length !== expected) {
          ...
          logger.warn('At-large congressional state has unexpected district count', {
            stateFips,
    ...
    // Validate required fields (ACTIVATING DEAD CODE: getExpectedFields)
    ...
    for (const boundary of boundariesToValidate) {
      ...
      if (missingFields.length > 0) {
        missingFieldsMap.set(boundary.geoid, missingFields);
      }
    }
    ...
    if (valid) {
      ...
      if (!countValid) {
        issues.push(`count mismatch (${actual}/${expected}, ${percentage.toFixed(1)}%)`);
      }
      ...
      if (!fieldsValid) {
        issues.push(`${missingFieldsMap.size} boundaries missing required fields`);
        // Add details about which fields are missing
        const fieldSummary = new Map<string, number>();
        for (const fields of missingFieldsMap.values()) {
          for (const field of fields) {
            fieldSummary.set(field, (fieldSummary.get(field) || 0) + 1);
          }
        }
        const topMissing = Array.from(fieldSummary.entries())
          .sort((a, b) => b[1] - a[1])
          .slice(0, 3)
  ...
  async validateAgainstShapefile(
    ...
    logger.warn('Shapefile validation not fully implemented', {
      message: 'Use as integration test with external GDAL tools',
    });
