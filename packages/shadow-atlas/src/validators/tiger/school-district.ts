...
import type { NormalizedBoundary } from './validator.js';
...
export class SchoolDistrictValidator {
  ...
  async computeCoverageWithoutStateBoundary(
    ...
    for (let i = 1; i < allBoundaries.length; i++) {
      try {
        ...
        logger.warn('Coverage analysis union operation failed', {
          geoid: allBoundaries[i].geoid,
  ...
  private async getStateBoundary(
    ...
    logger.info('Downloading state boundaries for coverage validation', {
      url,
      year,
    });
    ...
    logger.debug('Converting shapefile to GeoJSON');
    ...
    logger.debug('Cached state boundary', {
      cacheFile,
      stateFips,
    });
  ...
  async checkCoverage(
    ...
    for (let i = 1; i < allBoundaries.length; i++) {
      try {
        ...
        // Union failed - likely invalid geometry, skip this boundary
        logger.warn('School district coverage union operation failed', {
          geoid: allBoundaries[i].geoid,
          error: (error as Error).message,
        });
      }
    ...
    try {
      ...
      logger.warn('School district gap computation failed', {
        error: (error as Error).message,
      });
    }
