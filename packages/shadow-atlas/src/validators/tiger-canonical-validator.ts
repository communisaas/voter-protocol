...
export class TIGERCanonicalValidator {
  ...
  async validateLayer(
    ...
    console.log(`\nValidating ${layer.toUpperCase()} layer against canonical GEOIDs...`);
    console.log(`  States to validate: ${statesToValidate.length}`);
    ...
    for (const stateFips of statesToValidate) {
      try {
        ...
        console.log(
          `  ${result.stateAbbr ?? stateFips}: ${statusIcon} (${matchPct}% match, ` +
          `${result.missing.length} missing, ${result.extra.length} extra)`
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error(`  ${stateFips}: ERROR - ${message}`);
  ...
  async validateAllLayers(): Promise<CanonicalCrossValidationReport> {
    ...
    console.log('\n========================================');
    console.log('TIGER Canonical Cross-Validation Report');
    console.log('========================================');
    console.log(`Timestamp: ${timestamp}`);
    console.log(`TIGER Year: ${this.year}`);
