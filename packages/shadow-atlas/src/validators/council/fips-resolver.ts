...
export async function batchResolveFips(
  ...
  for (let i = 0; i < layers.length; i += concurrency) {
    ...
    if ((i + concurrency) % 50 === 0) {
      logger.info('FIPS resolution batch progress', {
        processed: Math.min(i + concurrency, layers.length),
        total: layers.length,
      });
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  import('node:fs').then(async (fs) => {
    ...
    logger.info('FIPS RESOLVER - Multi-Strategy Attribution');
    ...
    if (fs.existsSync(outputPath)) {
      try {
        ...
        logger.info('Loaded existing FIPS resolution results', {
          resolvedCount: existingResolved.length,
          unresolvedCount: existingUnresolved.length,
        });
      } catch (e) {
        logger.warn('Could not parse existing results, starting fresh');
      }
    }
    ...
    logger.info('FIPS resolution processing status', {
      remaining: remainingToProcess.length,
      total: results.needsReview.length,
    });

    if (remainingToProcess.length === 0) {
      logger.info('All layers already processed');
    } else {
      ...
      for (let i = 0; i < remainingToProcess.length; i += CHUNK_SIZE) {
        ...
        logger.info('Incremental FIPS resolution progress saved', {
          processedCount,
          totalRemaining: remainingToProcess.length,
          totalResolved: existingResolved.length,
        });
      }
    }

    logger.info('FIPS RESOLVER - Final Results', {
      resolvedCount: existingResolved.length,
      unresolvedCount: existingUnresolved.length,
      resolvedPercentage: ((existingResolved.length / results.needsReview.length) * 100).toFixed(1),
      outputPath,
    });
  });
}
