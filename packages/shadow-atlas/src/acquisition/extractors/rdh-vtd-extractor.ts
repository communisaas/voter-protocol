...
export class RDHVTDExtractor {
  ...
  async extractAll(states?: readonly string[]): Promise<readonly VTDExtractionResult[]> {
    ...
    logger.info('Processing states for VTD extraction', {
    ...
    for (const stateCode of targetStates) {
      try {
        ...
        logger.error('Error processing state VTD extraction', {
          stateCode,
          error: message,
        });
      }
  ...
  async extractState(stateCode: string): Promise<VTDExtractionResult | null> {
    ...
    if (!stateFips) {
      logger.warn('Unknown state code for VTD extraction', { stateCode });
      return null;
    }

    logger.info('Starting VTD extraction for state', {
      stateCode,
      stateFips,
    });
    ...
    if (!shpPath || this.forceRefresh) {
      ...
      if (datasets.length === 0) {
        logger.info('No VTD datasets available for state', { stateCode });
        return null;
      }

      logger.info('Found VTD datasets for state', {
        stateCode,
        datasetCount: datasets.length,
      });
      const best = datasets[0];
      logger.info('Selected VTD dataset', {
        stateCode,
        datasetTitle: best.Title,
      });
    ...
    logger.info('Processing shapefile', {
      stateCode,
      shapefile: basename(shpPath),
    });

    // Detect GEOID field
    const geoidField = await this.detectGeoidField(shpPath);
    if (!geoidField) {
      logger.error('Could not detect GEOID field in shapefile', {
        stateCode,
        shapefile: basename(shpPath),
      });
      return null;
    }
    logger.info('Detected GEOID field', {
      stateCode,
      geoidField,
    });
    ...
    logger.info('Extracted VTD GEOIDs', {
      stateCode,
      geoidCount: geoids.length,
    });
  ...
  private async listVTDDatasets(stateCode: string): Promise<readonly RDHDataset[]> {
    ...
    if (!Array.isArray(data)) {
      logger.error('RDH API error for state', {
        stateCode,
        error: (data as { message?: string }).message ?? String(data),
      });
      return [];
    }
  ...
  private async downloadFile(dataset: RDHDataset, stateCode: string): Promise<string> {
    ...
    logger.info('Downloading shapefile from RDH', {
      filename: dataset.Filename,
  ...
  private async extractGeoidsFromShapefile(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      logger.error('Error extracting GEOIDs from shapefile', {
        shapefile: basename(shpPath),
        geoidField,
        error: message,
      });
  ...
  private async writeStateJSON(result: VTDExtractionResult): Promise<void> {
    ...
    logger.info('Wrote VTD GEOID JSON file', { path: jsonPath });
  }
  ...
  private async writeManifest(results: readonly VTDExtractionResult[]): Promise<void> {
    ...
    logger.info('Wrote VTD extraction manifest', { path: manifestPath });
  }
...
export function createRDHVTDExtractor(
  ...
  if (!credentials) {
    logger.error('Missing RDH credentials', {
      message: 'RDH_USERNAME and/or RDH_PASSWORD environment variables not set',
    });
    return null;
  }
