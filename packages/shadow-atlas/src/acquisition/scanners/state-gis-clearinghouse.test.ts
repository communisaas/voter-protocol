...
describe('StateGISClearinghouseScanner - Recursive Service Exploration', () => {
  ...
  describe('exploreServicesRecursively', () => {
    it('should explore root-level services', async () => {
      ...
      // Mock 2: Root services response (exploreServicesRecursively fetches same URL again)
      fetchMock.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          folders: [],
      ...
      // Mock 3: Service exploration (CouncilDistricts)
      fetchMock.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
    ...
    it('should enforce max recursion depth', async () => {
      ...
      // Calls: 1 (REST endpoint discovery) + 5 (folder levels 0-4) = 6 total
    ...
    it('should filter layers by governance keywords', async () => {
      ...
      // Should have 1 candidate: Council Districts passes semantic validation (score ≥30)
      // Voting Precincts is rejected by negative keyword "voting" (score = 0)
      // Zoning Boundaries has no governance keywords (filtered out early)
      expect(result.length).toBe(1);
      expect(result[0].title).toBe('Council Districts');
    });
    ...
    it('should handle rate limiting correctly', async () => {
      ...
      // Mock 2: Root services response (has rate limit delay)
      fetchMock.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          folders: [],
      ...
      // - 100ms for root folder fetch
      // - 100ms for Service1 fetch
      // - 100ms for Service2 fetch
