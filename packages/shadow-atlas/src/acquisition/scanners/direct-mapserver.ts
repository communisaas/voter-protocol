...
export class DirectMapServerScanner {
  ...
  async search(city: CityTarget): Promise<PortalCandidate[]> {
    logger.info('Starting Direct MapServer scan', {
      city: city.name,
      state: city.state
    });
    ...
    logger.info('Generated GIS domain patterns', {
      city: city.name,
      state: city.state,
    ...
    for (const domain of domains.slice(0, this.maxDomainsPerCity)) {
      ...
      logger.debug('Checking GIS domain', {
        city: city.name,
        state: city.state,
        domain
      });
      ...
      try {
        ...
        logger.info('Found ArcGIS services on domain', {
          city: city.name,
          state: city.state,
          domain,
        ...
        for (const serviceUrl of services) {
          ...
          for (const layer of layers) {
            ...
            if (result.score >= 30) {
              logger.info('High-scoring layer discovered', {
                city: city.name,
                state: city.state,
    ...
    logger.info('Direct MapServer scan complete', {
      city: city.name,
      state: city.state,
      candidatesFound: candidates.length,
      domainsChecked: domainsChecked.length,
      servicesChecked,
      layersChecked
    });
