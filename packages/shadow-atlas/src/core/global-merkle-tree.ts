...
export class GlobalMerkleTreeBuilder {
  ...
  async build(districts: readonly GlobalDistrictInput[]): Promise<GlobalMerkleTree> {
    ...
    logger.info('Grouped districts by country', {
      countryCount: groupedByCountry.size,
    });
    ...
    logger.info('Built country trees', {
      countryTreeCount: countryTrees.length,
    });
    ...
    logger.info('Built continental trees', {
      continentalTreeCount: continentalTrees.length,
    });
    ...
    logger.info('Global Merkle tree build complete', {
      globalRoot: this.formatHash(globalRoot),
      totalDistricts: districts.length,
      durationMs: duration,
    });
  ...
  private getContinent(countryCode: string): ContinentalRegion {
    ...
    if (!region) {
      logger.warn('Unknown country code, defaulting to americas', {
        countryCode,
        message: 'Not found in ISO 3166-1 registry. Add to registry/iso-3166-countries.ts if this is a valid country.',
      });
      return 'americas';
    }
  ...
  private getCountryName(countryCode: string): string {
    ...
    if (!country) {
      logger.warn('Country code not found in registry', {
        countryCode,
        message: 'Add to registry/iso-3166-countries.ts if this is a valid country.',
      });
      return countryCode;
    }
