...
import { detectUpdates } from '../transformation/utils.js';
...
export function incrementalUpdate(
  ...
  // Convert new boundaries to normalized districts
  const newNormalizedDistricts = newBoundaries.map(
    extractedBoundaryToNormalizedDistrict
  );

  // Detect changes between existing and new
  const updateDetection = detectUpdates(
    existingTree.districts,
    newNormalizedDistricts
  );

  // Build map of new districts by ID for efficient merging
  const newDistrictsMap = new Map(
    newNormalizedDistricts.map(d => [d.id, d])
  );

  // Merge: Keep existing districts, update modified, add new, remove deleted
  const mergedBoundaries: NormalizedDistrict[] = [];

  // Add existing districts (excluding removed ones)
  for (const existingDistrict of existingTree.districts) {
    if (!updateDetection.removed.includes(existingDistrict.id)) {
      // Use new version if modified, otherwise keep existing
      const newVersion = newDistrictsMap.get(existingDistrict.id);
      if (newVersion) {
        mergedBoundaries.push(newVersion);
        newDistrictsMap.delete(existingDistrict.id); // Mark as processed
      } else {
        mergedBoundaries.push(existingDistrict);
      }
    }
  }

  // Add truly new boundaries (not in existing tree)
  for (const [id, newDistrict] of newDistrictsMap) {
    if (updateDetection.added.includes(id)) {
      mergedBoundaries.push(newDistrict);
    }
  }

  // Rebuild merkle tree with merged boundaries
  const builder = new MerkleTreeBuilder();
  const merkleTree = builder.build(mergedBoundaries);

  return {
    merkleTree,
    stats: {
      previousBoundaries: existingTree.districts.length,
      newBoundaries: updateDetection.added.length,
      updatedBoundaries: updateDetection.modified.length,
      totalBoundaries: mergedBoundaries.length,
    },
