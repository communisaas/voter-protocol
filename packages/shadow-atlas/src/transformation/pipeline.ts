...
import { generateMerkleTreeCID } from './utils.js';
...
export class TransformationPipeline {
  ...
  private rejectionSamples: RejectionSample[] = [];
  ...
  async transform(): Promise<TransformationResult> {
    ...
    // Generate IPFS CID for merkle tree
    const ipfsCID = await generateMerkleTreeCID(merkleTree);

    // Convert rejection tracking to plain object
    const rejectionReasonsObj: Record<string, number> = {};
    for (const [reason, count] of this.rejectionReasons) {
      rejectionReasonsObj[reason] = count;
    }
    ...
    const metadata: TransformationMetadata = {
      ...
      rejectionSamples: this.rejectionSamples.slice(0, 10), // First 10 samples
      ...
      for (const dataset of datasets) {
        ...
        if (semanticResult.score < 30) {
          ...
          this.trackRejection(
            ...
            `Score: ${semanticResult.score}/100. ${semanticResult.reasons.join(', ')}`
          );
        ...
        if (cityInfo) {
          ...
          if (!geoValidation.valid) {
            ...
            this.trackRejection(
              ...
              `${geoValidation.reason}. Expected: ${cityInfo.name}, ${cityInfo.state}`
            );
            continue; // Reject this dataset
          }
  ...
  /**
   * Track rejection with reason and sample
   *
   * @param id - Dataset identifier
   * @param reason - Rejection reason category
   * @param details - Detailed explanation
   */
  private trackRejection(id: string, reason: string, details: string): void {
    ...
    if (this.rejectionSamples.length < 10) {
      this.rejectionSamples.push({ id, reason, details });
    }
  }
}
