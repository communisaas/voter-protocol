...
export class TIGERExtractionService {
  ...
  async validate(result: TIGERLayerResult): Promise<ValidationResult> {
    ...
    // Extract GEOIDs
    const extractedGEOIDs = features.map((f) => f.id);

    // Perform GEOID completeness validation
    let geoidValidation: GEOIDValidationResult | null = null;
    const entityType = this.layerTypeToEntityType(layer);

    if (entityType && state) {
      // State-level validation
      geoidValidation = validateGEOIDCompleteness(state, entityType, extractedGEOIDs);
    } else if (entityType && !state && entityType !== 'place') {
      // National-level validation (places not supported at national level)
      geoidValidation = validateNationalGEOIDCompleteness(entityType, extractedGEOIDs);
    }

    // Combine validation results
    const missingGEOIDs = geoidValidation?.missing || [];
    const extraGEOIDs = geoidValidation?.unexpected || [];

    const valid = countValidation.isValid && (geoidValidation?.complete ?? true);

    let summary: string;
    if (valid) {
      summary = `✅ Valid: ${features.length}/${expected} features`;
    } else {
      const issues: string[] = [];
      if (!countValidation.isValid) {
        issues.push(`count mismatch (diff: ${countValidation.difference})`);
      }
      if (geoidValidation && !geoidValidation.complete) {
        if (missingGEOIDs.length > 0) {
          issues.push(`${missingGEOIDs.length} missing GEOIDs`);
        }
        if (extraGEOIDs.length > 0) {
          issues.push(`${extraGEOIDs.length} unexpected GEOIDs`);
        }
      }
      summary = `❌ Invalid: ${features.length}/${expected} features (${issues.join(', ')})`;
    }
   ...
   * Clear cache
   *
   * Removes cached extraction results.
   * Supports selective clearing by state and/or entity type.
   *
   ...
   *
   * // Clear only California data
   * await service.clearCache({ stateCode: '06' });
   *
   * // Clear only congressional district data
  ...
  async clearCache(options?: {
    stateCode?: string;
    entityType?: TIGERLayerType;
  }): Promise<{ clearedEntries: number; freedBytes: number }> {
    let clearedEntries = 0;
    let freedBytes = 0;

    // Clear CensusTigerLoader cache (TIGERweb REST API cache)
    this.loader.clearCache();

    // Clear result cache (stored extraction results)
    ...
    try {
      ...
      try {
        ...
        for (const file of files) {
          ...
          // Format: "state_{fips}_{layer}_{year}.json" or "national_{layer}_{year}.json"
          const match = file.match(/^(state_(\d+)|national)_([^_]+)_(\d+)\.json$/);
          if (!match) continue;

          const [, , stateFips, layer] = match;

          // Apply filters
          if (options?.stateCode && stateFips !== options.stateCode) {
            continue;
          }

          if (options?.entityType && layer !== options.entityType) {
            continue;
          }
        ...
        // Results directory doesn't exist or is empty - no cache to clear
        if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
          throw error;
        }
      }

      // Clear TIGERBoundaryProvider cache (shapefile downloads)
      if (!options || !options.stateCode) {
        // Only clear provider cache if clearing all or by entity type
        // (provider cache is organized by entity type, not state)
  ...
  /**
   * Clear provider cache (shapefile downloads)
   *
   * @param entityType - Optional entity type to filter by
   */
  private async clearProviderCache(entityType?: TIGERLayerType): Promise<void> {
    const { readdir, rm } = await import('node:fs/promises');

    try {
      const cacheEntries = await readdir(this.cacheDir, { withFileTypes: true });

      for (const entry of cacheEntries) {
        if (!entry.isDirectory()) continue;

        // Provider cache directories follow pattern: "cd_2024", "sldu_2024", etc.
        const match = entry.name.match(/^([a-z]+)_(\d+)$/);
        if (!match) continue;
        ...
        if (entityType) {
          const providerLayer = this.layerTypeToProviderLayer(entityType);
  ...
  /**
   * Map layer type to GEOID entity type
   */
  private layerTypeToEntityType(layer: TIGERLayerType): GEOIDEntityType | null {
    switch (layer) {
      case 'county':
        return 'county';
      case 'congressional':
      ...
      case 'cdp':
        return 'place';
      default:
        // School districts don't have standardized GEOID validation yet
        return null;
    }
  }
