...
describe('TIGER → Shadow Atlas Pipeline', () => {
  // WASM initialization is handled internally by Poseidon2Hasher singleton
  ...
  describe('Full Pipeline Integration', () => {
    ...
    it('rejects invalid proof with wrong boundary ID', async () => {
      ...
      const valid = await builder.verifyProof(wrongLeafProof);
  ...
  describe('IPFS Export', () => {
    ...
    it('exported JSON is deterministic', async () => {
      ...
      const tree1 = await builder.buildTree({ congressionalDistricts: caCD });
      const tree2 = await builder.buildTree({ congressionalDistricts: caCD });
  ...
  describe('Edge Cases', () => {
    it('handles empty layer gracefully', async () => {
      const builder = new MultiLayerMerkleTreeBuilder();

      // Empty congressional districts
      await expect(
        builder.buildTree({ congressionalDistricts: [] })
      ).rejects.toThrow('Cannot build Merkle tree: no leaves');
    });

    it('handles single boundary', async () => {
      const builder = new MultiLayerMerkleTreeBuilder();
      ...
      const tree = await builder.buildTree({ congressionalDistricts: [singleBoundary] });
    ...
    it('handles odd number of boundaries (unpaired leaf)', async () => {
      const builder = new MultiLayerMerkleTreeBuilder();
      ...
      const tree = await builder.buildTree({ congressionalDistricts: boundaries });
